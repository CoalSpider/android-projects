...
public class Util {
    ...
    public static float[] rayTriangleIntersect(float[] v1, float[] v2, float[] v3,
        ...
        Vec3f V1 = Vec3f.fromArray(v1);
        Vec3f V2 = Vec3f.fromArray(v2);
        Vec3f V3 = Vec3f.fromArray(v3);
        Vec3f VOrigin = Vec3f.fromArray(origin);
        Vec3f VDir = Vec3f.fromArray(dir);
        return rayTriangleIntersect(V1, V2, V3, VOrigin, VDir);
    }
     ...
     **/
    public static float[] rayTriangleIntersect(Vec3f v1, Vec3f v2, Vec3f v3,
                                               Vec3f origin, Vec3f dir) {
        Vec3f edge1, edge2; // edge1, edge2
        Vec3f pVec, qVec, tVec; // pqv
        float det, invDet, u, v, t;
        // find vectors for two edges sharing v1
        edge1 = Vec3f.sub(v2, v1);
        edge2 = Vec3f.sub(v3, v1);
        //Begin calculating determinant - also used to calculate u parameter
        pVec = Vec3f.cross(dir, edge2);
        ...
        tVec = Vec3f.sub(origin, v1);
        // calculate u parameter and test bound
        u = Vec3f.dot(tVec, pVec) * invDet;
        ...
        v = Vec3f.dot(dir, qVec) * invDet;
        // intersection lies outside the triangle
        if (v < 0f || u + v > 1f) {
            return null;
        }
        t = Vec3f.dot(edge2, qVec) * invDet;
        if (t > EPSILON) { // ray intersection
            Vec3f intersect = Vec3f.add(origin, Vec3f.scaleMult(dir, t));
            float w = 1 - u - v;
            return new float[]{
                    // intersection point
     ...
     **/
    public static List<Vec2f> drawLine(int width, int height, int x0, int y0, int x1,
                                       int y1, boolean
            lineOverlapMajor,
                                       boolean
                                               lineOverlapMinor) {
        ...
        List<Vec2f> xyPairs = new ArrayList<>();
        if (x0 == x1 && y0 == y1) {
            return xyPairs;
        }
        // vertical line
        ...
        if (dx > dy) {
            ...
            while (x0 != x1) {
                ...
                if (err >= 0) {
                    ...
                    // overlap minor
                    if (lineOverlapMinor) {
                        xyPairs.add(new Vec2f(x0 - stepX, y0));
                    }
                    err -= dxTimes2;
                }
                err += dyTimes2;
            ...
            while (y0 != y1) {
                ...
                if (err >= 0) {
                    ...
                    // overlap minor
                    if (lineOverlapMinor) {
                        xyPairs.add(new Vec2f(x0, y0 - stepY));
                    }
                    err -= dyTimes2;
                }
                err += dxTimes2;
     ...
     **/
    public static Vec2f cartesianToLogPolarSqrd(float centerX, float centerY, float
            x, float y) {
        float dx = x - centerX;
        float dy = y - centerY;
        // p (rho) = log((x-cX)^2+(y-cY)^2)
        float radius = (float) Math.log(dx * dx + dy * dy);
        // theta =  atan2(y - cY,x - cX)
        float angle = (float) Math.atan2(dy, dx);
        return new Vec2f(radius, angle);
    }

    /**
     * @return new Point2d(distance,angle in radians)
     **/
    public static Vec2f cartesianToPolar(float centerX, float centerY, float x, float
            y) {
        float dx = x - centerX;
        float dy = y - centerY;
        ...
        // theta =  atan2(y - cY,x - cX)
        float angle = (float) Math.atan2(dy, dx);
        return new Vec2f(distance, angle);
    }

    /**
     * @return new Point2d(distance,angle in radians)
     **/
    public static Vec2f polarToCartesian(float centerX, float centerY, float radius,
                                         float angle) {
        // x = radius * cos theta
        // y = radius * sin theta
        float cartesianX = (float) (radius * Math.cos(angle)) + centerX;
        float cartesianY = (float) (radius * Math.sin(angle)) + centerY;
        return new Vec2f(cartesianX, cartesianY);
    }
}
